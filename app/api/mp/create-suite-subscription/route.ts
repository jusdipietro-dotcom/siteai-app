...
export async function POST(req: NextRequest) {
  try {
    ...
    const planConfig = SUITE_JURIDICA_PLANS[sub.plan as keyof typeof SUITE_JURIDICA_PLANS]
    ...
    const backUrl = `${baseUrl}/suite-juridica?mp_return=true&sub=${subscriptionId}`
    const startDate = new Date(Date.now() + 120_000).toISOString()
    const extRef = `suite:${subscriptionId}:${sub.plan}`
